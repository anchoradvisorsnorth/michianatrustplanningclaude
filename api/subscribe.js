export default async function handler(req, res) {
  ...
  const CHECKLIST_URL    = isIN
    ? `${SITE}/checklists/indiana-trust-checklist.pdf`
    : `${SITE}/checklists/michigan-trust-checklist.pdf`;

  try {
    // 1 — Add subscriber to MailerLite group (with state field)
    await fetch('https://connect.mailerlite.com/api/subscribers', {
      ...
      body: JSON.stringify({ email, groups: [GROUP_ID], fields: { state: st.toUpperCase() } })
    });

    // 2 — Send state-specific checklist via Resend
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: email,
        subject: `Your ${stateName} Trust Readiness Checklist`,
        html: `
          <div style="font-family:'Helvetica Neue',sans-serif;max-width:560px;margin:0 auto;color:#253040">
            ...
            <div style="padding:36px 32px">
              <h1 style="font-size:22px;color:#1c2b3a;margin:0 0 16px;font-weight:600">Your ${stateName} Trust Readiness Checklist</h1>
              <p style="font-size:15px;line-height:1.7;color:#4a5568;margin:0 0 20px">
                Here's your checklist — 7 steps covering everything a ${stateName} family needs for a complete, properly funded trust plan.
              </p>
              ...
              <div style="background:#faf8f4;border-left:3px solid #b07d2e;padding:20px 24px;margin:32px 0 0;border-radius:0 6px 6px 0">
                ...
                <p style="font-size:14px;line-height:1.7;color:#4a5568;margin:0 0 14px">
                  The checklist shows you <em>what</em> to fix. The <strong>${stateName} Trust Planning Workbook</strong> walks you through <em>how</em> — naming trustees, structuring distributions, funding every asset correctly. It picks up right where the checklist leaves off.
                </p>
                <a href="${SITE}#step-workbook"
            ...
            <div style="background:#f5f1ea;padding:16px 32px;text-align:center">
              <p style="margin:0;font-size:11px;color:#9aa5b1">
                © 2026 Michiana Trust Planning · ${isIN ? 'Northern Indiana' : 'Southwest Michigan'}<br>
